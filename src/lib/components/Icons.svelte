<script lang="ts">
	let {
		...
		size = 20,
		class: className
	}: { name: string; color?: string; size?: number | string; class?: string } = $props();

	// Standardize name to lowercase single letter for easier mapping if needed, or keep full names
	// Incoming names: 'pawn', 'knight', 'bishop', 'rook', 'queen', 'king'
	// Map to p, n, b, r, q, k
	...
	let code = $derived(map[name.toLowerCase()] || 'p');
	let isWhite = $derived(color === 'w');
</script>
	...
	{:else if code === 'n'}
		{#if isWhite}
			<g
				...
				stroke-width="1.5"
				><path fill="#fff" d="M22 10c10.5 1 16.5 8 16 29H15c0-9 10-6.5 8-21" /><path
					fill="#fff"
		...
		{:else}
			<g
				...
				stroke-linejoin="round"
				stroke-width="1.5"
				><path fill="#000" d="M22 10c10.5 1 16.5 8 16 29H15c0-9 10-6.5 8-21" /><path
					fill="#000"
	...
	{:else if code === 'b'}
		{#if isWhite}
			<g
				...
				><path stroke-linejoin="miter" d="M17.5 26h10M15 30h15m-7.5-14.5v5M20 18h5" /></g
			>
		{:else}
			<g
				fill="none"
	...
	{:else if code === 'r'}
		{#if isWhite}
			<g
				...
				/><path d="m34 14-3 3H14l-3-3" /><path
					stroke-linecap="butt"
					stroke-linejoin="miter"
					d="M31 17v12.5H14V17"
				/><path d="m31 29.5 1.5 2.5h-20l1.5-2.5" /><path
					fill="none"
					stroke-linejoin="miter"
					d="M11 14h23"
				/></g
		...
		{:else}
			<g
				...
				/><path stroke-linecap="butt" stroke-linejoin="miter" d="M14 29.5v-13h17v13H14z" /><path
					stroke-linecap="butt"
	...
	{:else if code === 'q'}
		{#if isWhite}
			<g
				...
				/><path fill="none" d="M11.5 30c3.5-1 18.5-1 22 0M12 33.5c6-1 15-1 21 0" /></g
			>
		{:else}
			<g
				fill-rule="evenodd"
				...
				stroke-width="1.5"
				><g stroke="none"
					><circle cx="6" cy="12" r="2.75" /><circle cx="14" cy="9" r="2.75" /><circle
						cx="22.5"
						cy="8"
						r="2.75"
				...
				/><path
					stroke-linecap="butt"
					d="M9 26c0 2 1.5 2 2.5 4 1 1.5 1 1 .5 3.5-1.5 1-1.5 2.5-1.5 2.5-1.5 1.5.5 2.5.5 2.5 6.5 1 16.5 1 23 0 0 0 1.5-1 0-2.5 0 0 .5-1.5-1-2.5-.5-2.5-.5-2 .5-3.5 1-2 2.5-2 2.5-4-8.5-1.5-18.5-1.5-27 0z"
				/><path fill="none" stroke-linecap="butt" d="M11 38.5a35 35 1 0 0 23 0" /><path
					fill="none"
					stroke="#ececec"
	...
	{:else if code === 'k'}
		...
		{:else}
			<g
				...
				stroke-linejoin="round"
				stroke-width="1.5"
				><path stroke-linejoin="miter" d="M22.5 11.6V6" /><path
					fill="#000"
					stroke-linecap="butt"
					stroke-linejoin="miter"
					d="M22.5 25s4.5-7.5 3-10.5c0 0-1-2.5-3-2.5s-3 2.5-3 2.5c-1.5 3 3 10.5 3 10.5"
				...
				/><path stroke-linejoin="miter" d="M20 8h5" /><path
					stroke="#ececec"
